...
-- NOT NULL 제악조건 : 해당 컬럼에 null 을 포함하지 못하도록 함
...
-- NOT NULL 제약조건이 걸린 not_null_column 에 값을 지정하지 않음
INSERT INTO not_null_table (null_column) VALUE (1);

-- NOT NULL 이 지정된 컬럼은 INSERT시에 '반드시' null 이 아닌 값이 입력되어야함
INSERT INTO not_null_table (not_null_column) VALUE (1);
INSERT INTO not_null_table (not_null_column) VALUE (null);
-- NOT NULL 이 지정된 컬럼은 UPDATE시에 null 을 지정할 수 없음
...
-- UNIQUE 로 지정된 컬럼은 INSERT시에 중복된 데이터를 지정할 수 없음
INSERT INTO unique_table VALUES (1, 1);
INSERT INTO unique_table VALUES (2, 1);
-- UNIQUE 로 지정된 컬럼은 UPDATE시에 중복된 값으로 변경할 수 없음
UPDATE unique_table SET unique_column = 3;

-- NOT NULL + UNIQUE = 후보키
-- 후보키 : 레코드를 식별하기 위한 컴럼 (중복 x, null x)
CREATE TABLE candidate_table (
	candidate_coliunb INT NOT NULL UNIQUE,
    unique_column INT UNIQUE
);
...
-- 자신 테이블에서의 INSERT, UPDATE / 참조되어지는 테이블의 INSERT, UPDATE 에 영향을 미침
...
-- PRIMARY KEY 제약조건 : INSERT시에 NOT NULL의 조건과 UNIQUE 의 조건을 만족해야 함
INSERT INTO primary_table VALUES (null, null);
INSERT INTO primary_table VALUES (1, null);
INSERT INTO primary_table VALUES (1, null);   -- 중복되어 불가능

-- PRIMARY KEY 제약조건 : UPDATE시에 NOT NULL 조건과 UNIQUE 의 조건을 만족해야함
...
DELETE FROM primary_table WHERE primary_column = 1; -- 2 가능

DELETE FROM foreign_table;  -- 1
...
-- ON UPDATE : 참조하고있는 테이블의 기본키가 변경되었을 때 동작
-- ON DELETE : 참조하고있는 테이블의 기본키가 삭제되었을 때 동작

-- CASCADE : 참조하고 있는 테이블에서 데이터를 삭제하거나 수정했을 때, 참조하는 테이블에서도 삭제와 수정이 같이 일어남
-- SET NULL : 참조하고 있는 테이블에서 데이터를 삭제하거나 수정했을 때, 참조하는 테이블의 해당 데이터를 null 로 지정
...
-- CHECK 로 지정된 컬럼은 지정 조건에 부합하지 않으면 INSERT 불가능
INSERT INTO check_table VALUES (2, '남자');
-- CHECK 로 지정된 컬럼은 지정 조건에 부합하지 않으면 UPDATE 불가능
