...
SELECT (speed_80m + speed_76m + speed_70m + speed_35m) / 4 AS avg_speed
 ...
 -- XOR 은 OR 에서 AND 를 뺀 값
 
 -- LIKE 연산자: 문자열을 비교할 때 패턴을 기준으로 비교
 
 -- % : 임의의 개수(0 ~ 무한대)의 문자
 -- _ : 임의의 한 개 문자
 SELECT * FROM jeju WHERE observe_date LIKE '20%';  -- 20 으로 시작하는 문자열
 SELECT * FROM jeju WHERE observe_date LIKE '%08';  -- 08 으로 끝나는 문자열
 SELECT * FROM jeju WHERE observe_date LIKE '%08%';  -- 08 을 포함하는 문자열
 ...
 -- ASC : 오름차순 정렬
 -- DESC : 내림차순 정렬
 -- ORDER BY column1 [ASC|DESC], column2 [ASC|DESC], ...; 컬럼1을 정렬하고 정렬한것에 컬럼2를 정렬
